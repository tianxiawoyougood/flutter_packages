...
//        int timeout = [arguments[0] intValue];
        ...
        [UMCommonHandler debugLoginUIWithController:vc model:model complete:^(NSDictionary * _Nonnull resultDic) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [strongself.msgChannel invokeMethod:@"getLoginToken" arguments:resultDic];
            });
...
-(UMCustomModel *)getUMCustomModel:(NSDictionary *)dic
{
    ...
    if ([[dic allKeys] containsObject:@"contentViewFrame"]) {
        ...
        model.contentViewFrameBlock = ^CGRect(CGSize screenSize, CGSize contentSize, CGRect frame) {
            ...
            CGFloat x = [arr[0] doubleValue] < 0 ? frame.origin.x : [arr[0] doubleValue];
            CGFloat y = [arr[1] doubleValue] < 0 ? frame.origin.y : [arr[1] doubleValue];
            CGFloat w = [arr[2] doubleValue] < 0 ? frame.size.width : [arr[2] doubleValue];
            CGFloat h = [arr[3] doubleValue] < 0 ? frame.size.height : [arr[3] doubleValue];
    ...
    if ([[dic allKeys] containsObject:@"loginBtnFrame"]) {
        ...
        model.loginBtnFrameBlock = ^CGRect(CGSize screenSize, CGSize contentSize, CGRect frame) {
            ...
            CGFloat x = [arr[0] doubleValue] < 0 ? frame.origin.x : [arr[0] doubleValue];
            CGFloat y = [arr[1] doubleValue] < 0 ? frame.origin.y : [arr[1] doubleValue];
            CGFloat w = [arr[3] doubleValue] < 0 ? frame.size.width : [arr[3] doubleValue];
            CGFloat h = [arr[4] doubleValue] < 0 ? frame.size.height : [arr[4] doubleValue];
